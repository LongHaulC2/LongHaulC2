...
MAX_RETRIES = 10  # Max number of polls (6 * 5s = 30s total wait)
CURRENT_LISTENER_PORT = 7118
...
def step_5_execute_payload(filepath):
    ...
    Checks if process stays alive for at least 2 seconds.
    """
    try:
...
def get_existing_implant_uuids():
    ...
    try:
        ...
        if resp.status_code == 200:
            data = resp.json().get("data", [])
            # Return a set of UUIDs for fast lookups
            if data is None:
                return set()
            return {implant.get("implant_uuid") for implant in data}
    except Exception as e:
        log(f"Failed to fetch existing implants: {e}", "WARNING")
    return set()
...
def wait_for_implant_checkin(ignored_uuids):
    ...
    max_wait_time = MAX_RETRIES * SLEEP_INTERVAL
            ...
            if resp.status_code == 200:
                ...
                if implants:
                    for implant in implants:
                        ...
                        # THE FIX: Only return if it's NOT in our snapshot
                        if implant_uuid not in ignored_uuids:
                            log(
                                f"New Implant detected! UUID: {implant_uuid}", "SUCCESS"
                            )
                            return implant_uuid
    ...
    log("Timed out waiting for new implant.", "ERROR")
    return None


def step_6_queue_command(implant_uuid):
    """Step 6: Enqueue command (whoami)"""
    log(f"Queuing command for {implant_uuid}", "INFO")

    # Optional small wait to ensure implant is fully 'ready' in DB
    time.sleep(2)
...
def step_7_check_output(implant_uuid, task_uuid):
    ...
    log(f"Polling history for task {task_uuid}", "INFO")
    ...
    for i in range(MAX_RETRIES):
        try:
            ...
            if resp.status_code == 200:
                ...
                tasks = history_data.get("data", [])

                for task in tasks:
                    extracted_task_uuid = task.get("task_uuid")
                    extracted_task_response = task.get("task_response")

                    # Match the specific task we sent
                    if extracted_task_uuid == task_uuid:
                        if extracted_task_response:
                            log(
                                f"Task response received: {extracted_task_response}",
                                "SUCCESS",
                            )
                            return True
                        else:
        ...
        time.sleep(SLEEP_INTERVAL)
        log(f"   ...waiting for task response ({i+1}/{MAX_RETRIES})", "INFO")
        ...
        try:
            ...
            if not build_hash:
                test_report[profile_name] = (
                    "FAILURE: Step 3.5 (Build Job Failed/Timeout)"
                )
                continue
            ...
            if not process:
                test_report[profile_name] = (
                    "FAILURE: Step 5 (Execution - Crashed/Exited)"
                )
                continue

            # Wait for Check-in (Pass the snapshot to ignore old implants)
            implant_uuid = wait_for_implant_checkin(existing_uuids)
