...
These prefixes are **not** normal arguments — they trigger special resolution behavior before the command is sent to the implant. They are supported on commands that accept binary input: `file upload`, `bof`, and `memstore upload`.
...
```
bof fs:arp_scan.o "-subnet 10.0.0.0/24"
```
