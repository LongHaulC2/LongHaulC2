...
**Environment config:** All secrets and service addresses come from `.env` via `dotenv_values(".env")` in `server/instance.py`. The `.env` is loaded at server startup — no runtime reloads. The client also loads `.env` via `load_dotenv()` for `NICEGUI_STORAGE_SECRET` and SSL cert paths.

**SSL certificates:** Both the API (Gunicorn) and UI (NiceGUI) use self-signed certs in deploy mode, generated by `make certs`. Cert paths in `.env`:
- `API_CERT_FILE` / `API_CERT_KEY` — used by Gunicorn via systemd `EnvironmentFile`
- `UI_CERT_FILE` / `UI_CERT_KEY` — used by `client/main.py`; if both exist on disk, NiceGUI starts with HTTPS. In dev mode (certs absent), NiceGUI runs plain HTTP.
