...
# from client.src.client.modules.api_calls import get_file_data
...
async def file_details(file_uuid: str):
    ...
    api_res = {}  # await get_file_data(file_uuid)
    file_data = api_res.get("data", {}) or {}
...
async def render_dashboard(file_data: dict, file_uuid: str):
    ...
    file_size = file_data.get("file_size_kb", "0")
    md5_hash = file_data.get("md5", "UNKNOWN")
    first_seen = file_data.get("first_seen", "UNKNOWN")
    ...
    with ui.column().classes("w-full h-full gap-0 tech-glass-panel"):
        with ui.row().classes("tech-header-bar flex w-full items-center justify-between shrink-0"):
            with ui.row().classes("items-center gap-4"):
                ...
                ui.icon("description", size="md", color="emerald-500").classes(
        ...
        with ui.row().classes("w-full flex-grow p-4 gap-4 overflow-hidden no-wrap items-stretch"):  # noqa - nicegu
            with ui.column().classes(
                ...
                with ui.tab_panels(tabs, value="metadata_tab").classes("w-full flex-grow bg-transparent p-0"):
                    ...
                    with ui.tab_panel("preview_tab").classes(
                        "w-full h-full items-center justify-center text-neutral-600"
                    ):
                        ui.icon("data_object", size="xl").classes("mb-2 opacity-50")
                        ui.label("HEX PREVIEW MODULE NOT IMPLEMENTED").classes("tech-label-sub")
