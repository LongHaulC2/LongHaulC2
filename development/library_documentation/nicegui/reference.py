...
def _render_section(class_obj: type, attributes: list[Attribute], *, method_section: bool) -> None:
    ...
    if inherited_attributes:
        with ui.expansion(
            f'Inherited {"methods" if method_section else "properties"}', icon="account_tree", value=True
        ).classes(f"w-full {d.BORDER} rounded-xl").props("header-class=text-gray-500"):
...
def _render_attribute(item: Attribute, *, method_section: bool) -> None:
    if method_section:
        ...
        ui.markdown(f"{decorator}**`{item.name}`**`{_generate_method_signature_description(item.obj)}`").classes(
