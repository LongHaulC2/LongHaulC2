...
def code_window(code: str = "", *, title: str = "main.py", language: str = "python") -> ui.column:
    ...
    with ui.column().classes(f"rounded-xl gap-0 min-w-0 {d.BG_CODE} code-window") as window:
        with _header_row():
            ...
            if code:
                ui.space()
                with ui.button(on_click=lambda: ui.clipboard.write(code)).props("flat round size=xs").classes(
        ...
        if code:
            ui.markdown(f"````{language}\n{helpers.remove_indentation(code)}\n````").classes(
                "w-full grow py-2 overflow-x-auto [&_pre]:px-4 [&_pre]:w-fit [&_pre]:min-w-full"
            )
    return window
