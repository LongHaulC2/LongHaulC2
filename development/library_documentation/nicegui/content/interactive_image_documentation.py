...
def main_demo() -> None:
    ...
    def mouse_handler(e: events.MouseEventArguments):
        ...
        ii.content += (
            f'<circle cx="{e.image_x}" cy="{e.image_y}" r="15" fill="none" stroke="{color}" stroke-width="4" />'
        )
...
def adding_layers():
    ...
    def mouse_handler(e: events.MouseEventArguments):
        image.content += (
            f'<circle cx="{e.image_x}" cy="{e.image_y}" r="30" fill="none" stroke="red" stroke-width="4" />'
        )
...
def nesting_elements():
    with ui.interactive_image("https://picsum.photos/id/147/640/360"):
        ui.button(on_click=lambda: ui.notify("thumbs up"), icon="thumb_up").props("flat fab color=white").classes(
...
def crosshairs():
    ...
    ui.interactive_image("https://picsum.photos/id/565/640/360").add_slot(
        "cross",
        """
