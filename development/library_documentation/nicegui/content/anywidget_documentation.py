...
def main_demo() -> None:
    ...
    ui.button(on_click=counter.increment).bind_text_from(counter, "value", backward=lambda c: f"Count is {c}")


@doc.demo(
    "Altair charts with AnyWidget",
    """
...
def integrate_altair() -> None:
    ...
    chart = alt.Chart(df).mark_circle().encode(x="x", y="y", size="size", color="size", tooltip=["x", "y", "size"])
