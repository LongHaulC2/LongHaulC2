...
def state_demo():
    fullscreen = ui.fullscreen(on_value_change=lambda e: ui.notify("Enter" if e.value else "Exit"))
    ui.button("Toggle Fullscreen", on_click=fullscreen.toggle)
    ui.label().bind_text_from(fullscreen, "state", lambda state: "Fullscreen" if state else "")
