...
def customize_labels():
    ...
    ui.range(min=0, max=100, value={"min": 20, "max": 80}).props('label snap color="secondary"')

    ui.label("Customize the color of the labels")
    ui.range(min=0, max=100, value={"min": 40, "max": 80}).props(
        'label-always snap label-color="secondary" right-label-text-color="black"'
    )
