...
def register_event_handlers() -> None:
    ui.button("Python handler").on("click", lambda e: ui.notify(f'click: ({e.args["clientX"]}, {e.args["clientY"]})'))

    ui.button("JavaScript handler").on("click", js_handler="(e) => alert(`click: (${e.clientX}, ${e.clientY})`)")

    ui.button("Combination").on(
        "click", lambda e: ui.notify(f"click: {e.args}"), js_handler="(e) => emit(e.clientX, e.clientY)"
    )
