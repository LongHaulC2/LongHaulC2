...
def cancel_timer_demo():
    ...
    def start_progress():
        ...
        with ui.column() as controls:
            ui.button("Cancel").on("click", lambda: timer.cancel(with_current_invocation=False)).on(
                "click", controls.delete
            )
            ui.button("Cancel with current invocation").on(
                "click", lambda: timer.cancel(with_current_invocation=True)
